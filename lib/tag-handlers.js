...
module.exports = {
	'c:forEach': function (node, index, options, data, renderNode) {
		return '<!-- c:forEach (' + index + ') removed -->'
	},
	'c:if': tag_if_handler,
	's:if': tag_if_handler,
	's:else': function (node, index, options, data, renderNode) {
		if (!last_s_if_mathed) {
	...
	'default': function (node, index, options, data, renderNode) {
...
function tag_if_handler(node, index, options, data, renderNode) {
	var token = node.token
	...
	if (testExp && typeof testExp === 'string' && (testExp = testExp.trim())) {
		...
		let values = Object.assign({}, options.globals, data);
		var match = utils.evalExp(testExp, values)
