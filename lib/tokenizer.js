...
class Tokenizer {
    ...
    run(text) {
        ...
        while (this.pos < this.text.length - 1) {
            ...
            this.cursor = this.rest.indexOf('<');
            if (this.cursor === -1) {
                this.saveTextToken(this.pos, this.text.length - 1);
                break;
            }
    ...
    saveDirectiveToken(start, end) {
        ...
        this.tokens.push({
            type: 'directive',
            directive: directive,
            params: utils.getAttrs(textChunk.slice(directive.length)),
            start: start,
            end: end,
            text: this.text.slice(start, end),
            data: textChunk,
    ...
    saveCodeExpressionToken(start, end) {
        this.tokens.push({
            ...
            text: this.text.slice(start, end),
            data: this.text.slice(start + 3, end - 2).trim(),
            inTag: this.inTag
        });
        this.pos = end;
        this.rest = this.text.slice(end);
    ...
    saveCodeToken(start, end) {
        this.tokens.push({
            ...
            text: this.text.slice(start, end),
            data: this.text.slice(start + 2, end - 2).trim(),
            inTag: this.inTag
        });
        this.pos = end;
        this.rest = this.text.slice(end);
