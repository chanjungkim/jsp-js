...
class Renderer {
	...
	render(file, data) {
		...
		let html = this.renderChildren(ast, data);
		let element = this.renderEval(html, data);
	...
	renderChildren(parent, data) {
		return parent.children.map((child, i) => {
			return this.renderNode(child, i, data);
		}).join('');
	}
	...
	renderCustomTagNode(node, index, data) {
		...
		if (typeof this.options.tags[namespace] !== 'undefined'
			...
			return this.options.tags[namespace][funcname](node, index, this.options, this);
		}
